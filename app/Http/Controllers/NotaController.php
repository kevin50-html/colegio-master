...
class NotaController extends Controller
{
    ...
    {
        return redirect()->route('academico.horarios.actividades.show', [$actividad->horario, $actividad]);
    }
    ...
    public function create(Actividad $actividad): View|RedirectResponse
    {
        ...
        if (!$usuario || !$usuario->hasAnyPermission(['registrar_notas'])) {
            return redirect()->route('academico.horarios.actividades.show', [$actividad->horario, $actividad])->with('error', 'No tienes permisos para registrar notas.');
        }

        $cursoId = $actividad->horario->periodo->materia->curso_id;
    ...
    public function store(Request $request, Actividad $actividad): RedirectResponse
    {
        ...
        return redirect()->route('academico.horarios.actividades.show', [$actividad->horario, $actividad])->with('success', 'Nota registrada correctamente.');
    }
    ...
    public function edit(Actividad $actividad, Nota $nota): View|RedirectResponse
    {
        ...
        if ($nota->actividad_id !== $actividad->id) {
            return redirect()->route('academico.horarios.actividades.show', [$actividad->horario, $actividad])->with('error', 'La nota no pertenece a la actividad indicada.');
        }

        $usuario = Auth::user();

        if (!$usuario || !$usuario->hasAnyPermission(['registrar_notas'])) {
            return redirect()->route('academico.horarios.actividades.show', [$actividad->horario, $actividad])->with('error', 'No tienes permisos para editar notas.');
        }
    ...
    public function update(Request $request, Actividad $actividad, Nota $nota): RedirectResponse
    {
        ...
        return redirect()->route('academico.horarios.actividades.show', [$actividad->horario, $actividad])->with('success', 'Nota actualizada correctamente.');
    }
    ...
    public function destroy(Actividad $actividad, Nota $nota): RedirectResponse
    {
        ...
        if (!$usuario || !$usuario->hasAnyPermission(['registrar_notas'])) {
            return redirect()->route('academico.horarios.actividades.show', [$actividad->horario, $actividad])->with('error', 'No tienes permisos para eliminar notas.');
        }

        $nota->delete();

        return redirect()->route('academico.horarios.actividades.show', [$actividad->horario, $actividad])->with('success', 'Nota eliminada correctamente.');
    }
