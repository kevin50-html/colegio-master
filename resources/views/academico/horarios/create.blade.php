...
@section('title', 'Nuevo horario - ' . $cursoMateria->curso->nombre . ' / ' . $cursoMateria->materia->nombre)
            ...
            <div class="main-content p-4">
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-white">
                        <h2 class="h5 mb-0">Crear horario</h2>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('academico.curso-materias.horarios.store', $cursoMateria) }}" method="POST" class="row g-3">
                            @csrf
                            <div class="col-md-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="periodo_id" id="periodo_id" class="form-select @error('periodo_id') is-invalid @enderror">
                                    ...
                                    @foreach($periodos as $periodo)
                                        <option value="{{ $periodo->id }}" @selected(old('periodo_id') == $periodo->id)>{{ $periodo->nombre }}</option>
                                    @endforeach
                                </select>
                                @error('periodo_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="aula" id="aula" class="form-control @error('aula') is-invalid @enderror" value="{{ old('aula') }}" placeholder="Ubicación opcional">
                                @error('aula')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
