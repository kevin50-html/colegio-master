...
4. Identificación y eliminación de piezas sin uso ("welcome" y comando `inspire`).
5. Consolidación de hallazgos en este documento para dejar trazabilidad en español.
...
- `tests/Feature/ExampleTest.php` mantiene una prueba funcional mínima que verifica la redirección `/` → `/login` y que la vista de login responde correctamente. Sirve como smoke test para la autenticación. 【F:tests/Feature/ExampleTest.php†L9-L20】

## 9. Hallazgos y limpieza de código muerto
- Se eliminó la vista `resources/views/welcome.blade.php`, heredada del scaffolding de Laravel pero sin rutas asociadas (todas las entradas redirigen a `/login`). Suprimirla evita confusión al buscar plantillas activas.
- Se depuró `routes/console.php` retirando el comando `inspire`, que no estaba documentado ni enlazado con procesos internos. Dejar el archivo vacío con una nota evita la ejecución accidental de comandos obsoletos. 【F:routes/console.php†L1-L5】

## 10. Recomendaciones
1. Incorporar pruebas de autorización para los controladores críticos (`MatriculaAcudienteController`, `NotaController`, etc.) aprovechando los permisos definidos en `User::hasAnyPermission`.
2. Centralizar reglas de permisos mediante políticas de Laravel para reducir duplicación en los controladores.
3. Extender el dashboard con métricas reales aprovechando los conteos ya disponibles en los modelos (`withCount`).
4. Documentar en `README.md` el uso del disco `upload_disk` para matrículas (FTP por defecto) y agregar scripts de ejemplo para configurar credenciales.
